				...
				for j in range(7):
					...
					for character in mypath:
						...
						if character == '.':
							find_dot = count
						count +=1
					savepath = newpath+mypath[last_slash:find_dot]+str(i)+str(j)+mypath[find_dot:]
					image.save(savepath)
					im1 = (imread(savepath)[:,:,:3]).astype(float32)
					output = sess.run(prob, feed_dict = {x:[im1]})
					inds = argsort(output)[0,:]
					sorter_name = ""
					for char in class_names[inds[-1]]:
						if not(char==','):
							sorter_name= sorter_name+char
						else:
							break
					if not os.path.exists(newpath+'/'+sorter_name): os.makedirs(newpath+'/'+sorter_name)
					os.rename(savepath, newpath+'/'+sorter_name+'/'+mypath[last_slash:find_dot]+str(i)+str(j)+mypath[find_dot:])
